/**
 * Puppeteer test script to capture mobile layout screenshots
...
async function captureMobileScreenshots() {
  ...
  try {
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, 'mobile-light-mode-before.png'),
      fullPage: false
    });
    ...
    await page.screenshot({
      path: path.join(screenshotsDir, 'mobile-dark-mode-before.png'),
      fullPage: false
    });
    ...
    if (headerElement) {
      ...
      await headerElement.screenshot({
        path: path.join(screenshotsDir, 'mobile-header-closeup-before.png')
      });
    }
    ...
    console.log('\n--- Current Theme Toggle Position Info ---');
    console.log(JSON.stringify(themeToggleInfo, null, 2));

    console.log('\n✓ Screenshots captured successfully!');
    console.log(`  → ${path.join(screenshotsDir, 'mobile-light-mode-before.png')}`);
    console.log(`  → ${path.join(screenshotsDir, 'mobile-dark-mode-before.png')}`);
    console.log(`  → ${path.join(screenshotsDir, 'mobile-header-closeup-before.png')}`);
