#!/usr/bin/env bash
#
# claude.sh — AI CLI adapter for Claude Code
#
# Source this file to get the ai_run function.
# Usage:
#   source pipeline/adapters/claude.sh
...
# ── ai_run ──────────────────────────────────────────────────────────────
# Sends a prompt to Claude Code CLI with optional context files.
#
# Arguments:
#   $1  PROMPT_FILE   — file containing the prompt text
#   $2  OUTPUT_FILE   — file to write Claude's response to
...
ai_run() {
  ...
  # Call Claude Code CLI in print mode (-p) and capture output
  local tmp_out
  tmp_out=$(mktemp)
  if claude -p "$full_prompt" > "$tmp_out" 2>&1; then
    mv "$tmp_out" "$output_file"
    echo "OK: response written to $output_file"
  else
    local rc=$?
    echo "ERROR: claude CLI exited with code $rc" >&2
    cat "$tmp_out" >&2
    rm -f "$tmp_out"
    return $rc
  fi
